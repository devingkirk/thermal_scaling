...
omega.c <- runif(dim(omega.matrix)[1], min=0.5,max=1.3)
omega.tmin <- runif(dim(omega.matrix)[1], min=0,max=10)
omega.tmax <- omega.tmin + runif(dim(omega.matrix)[1], min=15, max=35)

for(j in 1:dim(omega.matrix)[1]){
  for(i in 1:length(temp)){
    ifelse(temp[i]<omega.tmin[j] || temp[i]>omega.tmax[j],      
           omega.matrix[j,i]<-0,
           omega.matrix[j,i]<- (-omega.c[j]*(temp[i]-omega.tmin[j])*(temp[i]-omega.tmax[j]) ) / 2 )
  }
}
...
write.csv(data.mat, file="~/enviro_simulations_neither_proportional.csv")
